...
If you are trying to view the syslog but

$ tail -n 50 /var/log/syslog

is not showing recent messages, we suggest using

$ dmesg -wH

this will show the syslog in real time.

=== Tests ===

- We include files with what the output should look like under the test directory. 

- When you run procAncestry, there are two main outputs, either a successful one, 
  or one where the user is informed that they supplied an invalid pid.
  These can be found under tests/procAncestryValid.txt and tests/procAncestryInvalid.txt

- We also include what the syslog should show for various scenarios in phase 1. These files can be found in
  - tests/syslogReadMalicious.txt
  - tests/syslogOpen.txt
  - tests/syslogClose.txt
